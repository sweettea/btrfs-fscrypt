...
({									      \
	if (unlikely(!IS_ALIGNED(btrfs_file_extent_##name((leaf), (fi)), (alignment)))) \
...
static inline u64 file_extent_end(struct extent_buffer *leaf,
			   struct btrfs_key *key,
...
"unaligned file_offset for file extent, have %llu should be aligned to %u",
	...
	if (unlikely(btrfs_file_extent_type(leaf, fi) >= BTRFS_NR_FILE_EXTENT_TYPES)) {
		file_extent_err(leaf, slot,
	 ...
	 */
	if (unlikely(btrfs_file_extent_compression(leaf, fi) >= BTRFS_NR_COMPRESS_TYPES)) {
		file_extent_err(leaf, slot,
			   ...
			   int slot)
{
	...
	if (item_key.type == BTRFS_XATTR_ITEM_KEY) {
		if (unlikely(key->type != 0 || key->objectid != 0 || key->offset != 0))
			return -EUCLEAN;
		return 0;
	}
			  ...
			  int slot)
{
	 ...
	 * be a valid tree.
	 */
	if (unlikely(key->objectid == BTRFS_TREE_RELOC_OBJECTID && key->offset == 0)) {
			    ...
			    struct btrfs_key *key, int slot)
{
	 ...
	 */
	if (unlikely(!has_single_bit_set(mode & S_IFMT))) {
...
"inline ref item overflows extent item, ptr %lu iref size %u end %lu",
		...
		case BTRFS_SHARED_BLOCK_REF_KEY:
			if (unlikely(!IS_ALIGNED(inline_offset, fs_info->sectorsize))) {
				extent_err(leaf, slot,
		...
		case BTRFS_EXTENT_DATA_REF_KEY:
			...
			if (unlikely(!IS_ALIGNED(dref_offset, fs_info->sectorsize))) {
				extent_err(leaf, slot,
...
static int check_leaf(struct extent_buffer *leaf, bool check_item_data)
{
	...
	"bad key order, prev (%llu %u %llu) current (%llu %u %llu)",
		...
		if (unlikely(btrfs_item_nr_offset(slot) + sizeof(struct btrfs_item) >
