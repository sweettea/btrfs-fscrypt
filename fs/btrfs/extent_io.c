...
static void submit_pending_bio(struct btrfs_bio_ctrl *bio_ctrl, int mirror_num)
{
	/*
	 * Clean up of bio is handled by its endio function.
...
static void btrfs_iv_add(u8 *dst, const u8 *src, int ivsize, u64 addend)
{
	int i;
	__le64 *d = (__le64 *)dst, *s = (__le64 *)src;

	ASSERT(ivsize % sizeof(u64) == 0);
	for (i = 0; i < ivsize / sizeof(u64); i++) {
		u64 word = le64_to_cpu(s[i]);
		d[i] = cpu_to_le64(word + addend);
		/* Carry bit. */
		addend = word + addend < word;
	}
}
...
next:
		...
		bio_offset += len;
		btrfs_iv_add(bbio->iv, bbio->iv, bbio->ivsize,
			 ...
			 const u8 *iv, int ivsize)
{
	...
	if (bio_op(bio) == REQ_OP_READ && ivsize) {
		btrfs_bio(bio)->ivsize = ivsize;
		memcpy(btrfs_bio(bio)->iv, iv, ivsize);
	}
 ...
 * submit_pending_bio(bio_ctrl, mirror_num).
 *
 *
	...
	while (cur < pg_offset + size) {
		...
		if (compress_type != BTRFS_COMPRESS_NONE)
			...
			added = btrfs_bio_add_page(bio_ctrl, page,
					...
					pg_offset + offset, compress_type, iv, ivsize);
			  ...
			  struct extent_page_data *epd)
{
	...
	if (!btrfs_try_tree_write_lock(eb)) {
		submit_pending_bio(&epd->bio_ctrl, 0);
		flush = 1;
		btrfs_tree_lock(eb);
	}
				      ...
				      int mirror_num)
{
	...
	// why are we assuming the bio is already set up? we're then passing force_bio_submit in.
	// ah, but we don't have already allocated a bio attached to bio_ctrl so we don't go down
	// the fast path. so why is the force param true??
