			...
			const u8 *raw_key, const struct fscrypt_info *ci)
{
	...
	err = crypto_skcipher_setkey(tfm, raw_key, ci->ci_mode->keysize);
	if (err) {
		crypto_free_skcipher(tfm);
		return err;
	}
...
static int setup_new_mode_prepared_key(struct fscrypt_master_key *mk,
				       ...
				       u8 hkdf_context,
				       bool inlinecrypt, bool include_fs_uuid)
{
 ...
 */
static int find_and_lock_master_key(struct fscrypt_info *ci,
